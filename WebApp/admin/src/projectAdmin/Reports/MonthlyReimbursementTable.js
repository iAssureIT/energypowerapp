...
import '../ViewOption/ViewOption.css';
import 'bootstrap/dist/css/bootstrap.min.css';
import 'bootstrap/js/modal.js';
var sum = 0;
class MonthlyReimbursementTable extends Component {
	constructor(props){
		super(props);
		this.state = {
  ...
  modalClickEvent(){
    ...
    handleClick(event){
        ...
        this.props.history.push(tableObjects.viewdataurl+id);
	}
	handleChange(event){
		event.preventDefault();
		const name   = event.target.name;
		var remainingreimbursement = event.target.getAttribute('remainingreimbursement');
		
		if(event.target.value > remainingreimbursement){
			swal('Amount should not be greater than remaining reimbursement');
		}else{
			console.log("remainingreimbursement",remainingreimbursement);
	...
	onKeyPress(){
		var number = document.getElementById('paid');
    ...
    paidRequest(event){
        ...
        var formValues ={
            ...
            person_id  : event.currentTarget.id,
            paid       : parseFloat(this.state[event.currentTarget.id]),              
            updatedBy  : localStorage.getItem("user_ID")  
          }
          console.log("formValues",formValues);
          axios.post('/api/monthlyreimbursement/post', formValues)
          .then((response)=>{
			  console.log("this.state[event.currentTarget.id]",this.state[id]);
	                            ...
	                            (this.state.tableData && this.state.tableData.length > 0 ?
	                           		this.state.tableData.map( 
										(value, i)=> {
											return(
												<tr key={i} className="">
													<td className="textAlignCenter" style={{"textAlign": "center","verticalAlign": "middle"}}>{this.state.startRange+1+i} </td>
													{
														Object.entries(value).map( 
															([key, value1], i)=> {
																...
																if(found.length > 0){
																	if(key !== 'id'){
                                                                        if(key==="processofpayment"){
                                                                            if(value.totalreimbursement!=="0"){
                                                                                return(<td className="textAlignCenter" style={{"textAlign": "center","verticalAlign": "middle"}}>
                                                                                    <div className='input-group' style={{'width':'100%'}}>
                                                                                            <span className="input-group-addon"><i className="fa fa-rupee"></i></span>
                                                                                            <input type="number" className="form-control" ref="paid" min="0" name={value._id} value={this.state[value._id]} remainingreimbursement={value.remainingreimbursement} id="paid" onKeyDown={this.onKeyPress.bind(this)} onChange={this.handleChange.bind(this)} style={{'width':"80px"}}/>
                                                                                ...
                                                                                return(<td className={textAlign} style={{"textAlign": "center","verticalAlign": "middle"}}  key={i}><div className={textAlign}>NA</div></td>); 
                                                                            }
                                                                        }else{
                                                                            return(<td className={textAlign} style={{"textAlign": "center","verticalAlign": "middle"}}  key={i}><div className={textAlign} dangerouslySetInnerHTML={{ __html:value1}}></div></td>); 						
                                                                        }
																	}
																}
													...
													{value.showTracking ?
														<td className="textAlignCenter" style={{"textAlign": "center","verticalAlign": "middle"}}>
														    <span>
																	<i className="fa  fa-arrow-circle-right tablearrow" title="View"  id={this.state.tableData[i]._id} onClick={this.handleClick.bind(this)}></i>
...
export default withRouter(MonthlyReimbursementTable);