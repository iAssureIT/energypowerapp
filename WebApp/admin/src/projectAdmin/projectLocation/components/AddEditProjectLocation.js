...
import Division         from '../../Master/Division/Division.js';
import Industry               from '../../Master/Industry/Industry.js';
import Process          from '../../Master/Process/Process.js';
...
export default class AddEditProjectLocation extends Component{
  constructor(props) {
    super(props);
    this.state = {
            imgs                      :[],
  ...
  componentDidMount(){
    ...
    $("#recordform").validate({
      ...
      errorPlacement: function (error, element) {
        ...
        if (element.attr("name") == "divisionVal") {
          error.insertAfter("#divisionVal");
        }
        if (element.attr("name") == "processVal") {
          error.insertAfter("#processVal");
        }
        if (element.attr("name") == "industryVal") {
          error.insertAfter("#industryVal");
        }
  ...
  setDropdowns(){
   ...
   edit(id){
    axios({
      method: 'get', 
      url: '/api/projectlocation/get/one/'+id,
    }).then((response)=> {
      ...
      this.setState({
        ...
        "projectLocAddress"       : editData.projectLocAddress,
        "country"                   : editData.country,
        "images"                    : editData.images,
        "divisionVal"           : editData.division,
        "industryVal"                  : editData.industry,
        "processVal"               : editData.process,
  ...
  handleSubmit(event){
    ...
    var formValues = {
            ...
            industry                     : this.state.industryVal,
            process               : this.state.processVal,
          ...
          axios.post('/api/projectlocation/post', formValues)
          .then((response)=>{
            if(response.data.created){
              swal("Project Location "+this.state.locationName+" added successfully!");
               this.setState({
                ...
                this.props.history.push('/listprojectloc')
   ...
   handleUpdate(event){
    event.preventDefault();
     if ($('#recordform').valid()) {
      ...
      axios.patch('/api/projectlocation/update/'+this.state.editId,formValues)
        .then((response)=>{
...
deleteDoc(event){
  ...
  getDivision(){
  axios.post("/api/divisionMaster/get/list")
        .then((response)=>{
          this.setState({
              divisionArray : response.data
          })
          $('#RecoredrTypeData').val(this.state.divisionVal);
        })
        .catch((error)=>{
            console.log('error', error);
        })
  ...
  axios.post("/api/industryMaster/get/list")
    .then((response)=>{
      ...
      $('#IndustryData').val(this.state.industryVal);
    })
    .catch((error)=>{
        console.log('error', error);
    })
  ...
  getProcess(){
  axios.post("/api/process/get/list")
    .then((response)=>{
      this.setState({
          processArray : response.data
      })
      $('#MaxChannelData').val(this.state.processVal);
    })
    .catch((error)=>{
      console.log('error', error);
    })
  ...
  recorderModalClickEvent(){
    $('#divisionrModalId').addClass('in');
    $('#divisionrModalId').css('display','block');
    $('body').removeClass('modal-open');
    $('.modal-backdrop').remove();
  }
  recorderIndustryModalClickEvent(){
  ...
  channelModalClickEvent(){
    $('#processModalId').addClass('in');
    $('#processModalId').css('display','block');
    $('body').removeClass('modal-open');
    $('.modal-backdrop').remove();
  }
    ...
    render(){
      ...
      return(
        <div className="container-fluid">
                               ...
                               <div className="form-margin col-lg-12 col-md-12 col-sm-12 col-xs-12 zzero">
                                ...
                                <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 mainDiv NOpadding">
                                  <div className="col-lg-4 col-md-4 col-sm-12 col-xs-12 shippingInput">
                                    ...
                                    <div className={!this.props.modal&& "input-group"} id="divisionVal" > 
                                      <select ref="divisionVal" name="divisionVal" 
                                          value={this.state.divisionVal}  onChange={this.handleChange.bind(this)}
                                          className="form-control  col-lg-12 col-md-12 col-sm-12 col-xs-12 selectbox">
                                            <option disabled selected>--Select-- </option>
                                            {
                                              this.state.divisionArray && this.state.divisionArray.length > 0 ?
                                              this.state.divisionArray.map((recorderdata, index)=>{
                                                return(      
                                                      <option key={index}  value={recorderdata.division}>{recorderdata.division}</option>
                                                )}
                                              ) : ''
                                            }
                                      </select>
                                      {!this.props.modal&&<div className="input-group-addon inputIcon plusIconBooking" data-toggle="modal"   data-target="#divisionrModalId"  onClick={this.recorderModalClickEvent.bind(this)} title="Add Division" ><i className="fa fa-plus "></i>
                                      </div>}
                                    </div>  
                                  </div>
                                  <div className="col-lg-4 col-md-4 col-sm-12 col-xs-12 shippingInput" >
                                   <label className="col-lg-12 col-md-12 col-sm-12 col-xs-12 NOpadding labelform">Industry<span className="astrick">*</span></label>
                                      <div className={!this.props.modal&& "input-group"} id="industryVal" >  
                                       <select ref="industryVal" name="industryVal" 
                                        value={this.state.industryVal}  onChange={this.handleChange.bind(this)}
                                         className="form-control  col-lg-12 col-md-12 col-sm-12 col-xs-12 selectbox">
                                          <option disabled>--Select--</option>
                                           {
                                        ...
                                        {!this.props.modal&&<div className="input-group-addon inputIcon plusIconBooking" data-toggle="modal"   data-target="#industryModalId"  onClick={this.recorderIndustryModalClickEvent.bind(this)} title="Add Industry" ><i className="fa fa-plus "></i>
                                        </div>}
                                    </div> 
                                  </div> 
                                  <div className="col-lg-4 col-md-4 col-sm-12 col-xs-12 shippingInput" >
                                    <label className="col-lg-12 col-md-12 col-sm-12 col-xs-12 NOpadding labelform">Process<span className="astrick">*</span></label>
                                    <div className={!this.props.modal&& "input-group"} id="processVal" >  
                                      <select  ref="processVal" name="processVal" 
                                        value={this.state.processVal}  onChange={this.handleChange.bind(this)}
                                         className="form-control  col-lg-12 col-md-12 col-sm-12 col-xs-12 selectbox">
                                         
                                          <option disabled>--Select--</option>
                                           {
                                            this.state.processArray && this.state.processArray.length > 0 ?
                                            this.state.processArray.map((maxdata, index)=>{
                                              return(      
                                                  <option key={index} value={maxdata.process}>{maxdata.process}</option>
                                              );
                                            }
                                          ) : ''
                                        }
                                      </select>
                                        {!this.props.modal&&<div className="input-group-addon inputIcon plusIconBooking" data-toggle="modal"   data-target="#processModalId"  onClick={this.channelModalClickEvent.bind(this)} title="Add Industry" ><i className="fa fa-plus "></i>
                                        </div>}
                                    </div> 
                                  </div>
                                </div>   
          ...
          <div className="modal" id="divisionrModalId" role="dialog">
              ...
              <div className="modal-content adminModal-content col-lg-8 col-lg-offset-2 col-md-6 col-md-offset-3 col-sm-10 col-sm-offset-1 col-xs-12 noPadding">
                <div className="modal-header adminModal-header col-lg-12 col-md-12 col-sm-12 col-xs-12">
                  <div className="adminCloseCircleDiv pull-right  col-lg-1 col-lg-offset-11 col-md-1 col-md-offset-11 col-sm-1 col-sm-offset-11 col-xs-12 NOpadding-left NOpadding-right">
                      <button type="button" className="adminCloseButton" data-dismiss="modal" onClick={this.getDivision.bind(this)}>&times;</button>
                 </div>
                </div>
                <div className="modal-body adminModal-body col-lg-12 col-md-12 col-sm-12 col-xs-12">
                  <Division  editId={this.props.match && this.props.match.params.fieldID} editUrl= {this.props.match && this.props.match.params.id ? '/projectlocationmodal/'+this.props.match.params.id :'/projectlocationmodal'} />
                </div>  
             </div>
            </div>
          </div>
          <div className="modal" id="industryModalId" role="dialog">
            <div className="adminModal adminModal-dialog marginTopModal col-lg-12 col-md-12 col-sm-12 col-xs-12">
              <div className="modal-content adminModal-content col-lg-8 col-lg-offset-2 col-md-6 col-md-offset-3 col-sm-10 col-sm-offset-1 col-xs-12 noPadding">
                ...
                <div className="modal-body adminModal-body col-lg-12 col-md-12 col-sm-12 col-xs-12">
                  <Industry editId={this.props.match && this.props.match.params.fieldID} editUrl= {this.props.match && this.props.match.params.id ? '/projectlocationmodal/'+this.props.match.params.id :'/projectlocationmodal'} />
                </div>  
             </div>
            </div>
          </div>
          <div className="modal" id="processModalId" role="dialog">
            <div className="adminModal adminModal-dialog marginTopModal col-lg-12 col-md-12 col-sm-12 col-xs-12">
              <div className="modal-content adminModal-content col-lg-8 col-lg-offset-2 col-md-6 col-md-offset-3 col-sm-10 col-sm-offset-1 col-xs-12 noPadding">
                <div className="modal-header adminModal-header col-lg-12 col-md-12 col-sm-12 col-xs-12">
                  <div className="adminCloseCircleDiv pull-right  col-lg-1 col-lg-offset-11 col-md-1 col-md-offset-11 col-sm-1 col-sm-offset-11 col-xs-12 NOpadding-left NOpadding-right">
                      <button type="button" className="adminCloseButton" data-dismiss="modal" onClick={this.getProcess.bind(this)}>&times;</button>
                 </div>
                </div>
                <div className="modal-body adminModal-body col-lg-12 col-md-12 col-sm-12 col-xs-12">
                  <Process  editId={this.props.match && this.props.match.params.fieldID} editUrl= {this.props.match && this.props.match.params.id ? '/projectlocationmodal.</div></div></div></div></div>/'+this.props.match.params.id :'/projectlocationmodal'} />
                </div>  
             </div>
            </div>
          </div>
